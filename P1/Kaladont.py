...
def izberi_besedo(beseda, slovar):
    prejsna = ""
    for trenutna in slovar:
        if lahko_sledi(beseda, trenutna):
            if len(trenutna) > len(prejsna):
                prejsna = trenutna
            elif len(trenutna) == len(prejsna):
                if trenutna[1] < prejsna[1]:
                    return trenutna
                else:
                    return prejsna
            else:
...
def preveri_zaporedje(besede):
    ...
    return a and ni_ponavljanj(besede)
